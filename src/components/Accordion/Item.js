...
const Item = ({ name, id, child, active, toggle, children }) => {
    ...
    <div className='bg-white border border-gray-300 overflow-hidden'>
      <div className='meta pl-4 py-2 relative flex justify-between items-center'>
        ...
        <button className='link-wrapped pr-5 focus:outline-none' onClick={() => toggle(id)}>
          {
            child && child?.length > 0 && (<ArrowDown className={['transition-all duration-300 transform', active
              === id ? "rotate-180" : "rotate-0"].join(" ")}></ArrowDown>
            )}
        </button>
      </div>
      <div className='transition-all duration-500' style={{ height: active === id ? Height : 0 }}>
        <CSSTransition in={active === id} timeout={500} onEnter={calceHeight} classNames='accordion-item'>
